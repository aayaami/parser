...
class EsoreiterRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="article_wrapper"]');
        ...
        $this->removeDomNodes($newsPostCrawler, '//p[1][descendant::img[@class="article_image"]]
        | //*[@class="image_label"]
        | //i[contains(text(), "(esoreiter.ru)")]');

        $descriptionCrawler = $newsPostCrawler->filterXPath('//p[1]');
