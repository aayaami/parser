...
class LigaNewsNetParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="news-content"]');
        ...
        $descriptionCrawler = $newsPostCrawler->filterXPath('//div[@itemprop="description"]');
        if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text() !== '') {
            $previewNewsItem->setDescription($descriptionCrawler->text());
            $this->removeDomNodes($newsPostCrawler, '//div[@itemprop="description"]');
        }

        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," news-text ")]');

        // $this->removeDomNodes($contentCrawler, '//*[@class="pvc_clear"]/following-sibling::*');
